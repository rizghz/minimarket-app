...
<div class="row">
  <div class="col-md-12">
    <div class="enigma-dark-bg-1 shadow bgc-white bd bdrs-3 px-0 pt-1 pb-3 mB-10">
      @include('master.supplier.components.dialog')
...
$(() => {
  ...
  let dataId = "";
  let methodForm = "";
  const input = $("#form-supplier").find(".input");

  $("body").on("click", ".trigger", function(event) {
    event.preventDefault();
    let mode = $(this).data("mode");
    if (mode == "tambah") {
      $("#modal-label").text("Tambah Data Supplier");
      methodForm = "post";
    ...
    if (mode == "edit") {
      $("#modal-label").text("Edit Data Supplier");
      methodForm = "patch";
      let data = $(this).data('supplier');
      dataId = data.id;
      for (let i = 0; i < input.length; i++) {
        input.eq(i).val(
